...
/* 자료형
    NUMBER : 숫자형(정수, 실수)
    ** CHAR(크기) : 고정길이 문자형 -> 문자열 (2000BYTE) // 자바에서의 char랑 다름  
        -> ex) CHAR(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공간을 모두 사용.
        // 속도적인 측면에서 좋음 (주민등록번호, 전화번호, y/n 플래그)
        
    ** VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE) variable char 2 
...
INSERT INTO "MEMBER"
VALUES('MEM03','1QW2E423','박철수','970101-8970987', DEFAULT);
...
VALUES('MEM04','QQQQ111222','신짱구','900304-1231234');
 ...
 * [CONSTRAINT 제약조건명] 제약조건(지정할 컬러명)
 * */

/* NOT NULL 제약조건은 "컬럼 레벨"로만 설정이 가능하다!!
 * */
CREATE TABLE USER_USED_NN (
    -- USER_NO NUMBER NOT NULL, -- 제약조건 명 미지정 시 "SYS~" 형태로 자동 지정 
...
CREATE TABLE USER_USED_UK(
    ...
    -- 테이블 레벨로 USER_ID 컬럼 UINQUE 제약조건 설정 
...
CREATE TABLE USER_USED_UK2(
    ...
    CONSTRAINT USER_ID_NAEM_U UNIQUE(USER_ID, USER_NAME)
);
...
-- 두 컬럼이 모두 중복되는 값일 경우에만 오류 발생 -- 두 컬럼의 값이 중복될 경우, 오류 발생
...
CREATE TABLE USER_USED_PK(
    ...
    -- 테이블 레벨
    -- , CONSTRAINT USER_NO_PK_PRIMARY KEY(USER_NO)
);
...
CREATE TABLE USER_GRADE(
  GRADE_CODE NUMBER PRIMARY KEY, -- FK 제약조건으로 첨조할 수 있는 대상
...
-- GENDER 컬럼에 CHECK 제약조건으로 '남' 또는 '여'만 기록 가능한데 '남자'라는
-- 조건 이외의 값이 들어와 에러 발생
...
INSERT INTO CHECK_TEST VALUES('짱구', 300);
-- 체크 제약조건(KH11_PJS.CHECK_SCORE)이 위배되었습니다
...
DROP TABLE USER_TEST; -- 잘못 만들었을때 한번씩 실행 
...
INSERT INTO USER_TEST
	VALUES(1, 'user1', 'pass01', '12341234', '123456-1234567', '여', '01012341234', '서울특별시 중구 ', 'N'
);
...
--> 서브쿼리로 테이블 생성 시
--  별도의 제약조건 설정, DEFAULT 설장, COMMENT 설정 등이 필요! 
 ...
 * 
 * NOT NULL은 
 * 컬럼의 상태 (NULL허용, NULL 불가)를 수정(MODIFY)하는 모습 
 * 
 * */
