...
SELECT 
	E.EMP_ID , E.EMP_NAME , J.JOB_CODE , E.DEPT_CODE , E.SALARY , E.HIRE_DATE 
FROM 
	EMPLOYEE E
JOIN 
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
-- HAVING : 
-- GROUP BY절에 의해 만들어진 그룹 중 조회를 원하는 그룹에 대한 조건
...
SELECT 
	EMP_NO, EMP_NAME, DEPT_CODE, E.JOB_CODE	
FROM
	EMPLOYEE E
JOIN 
	DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN 
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
WHERE (DEPT_CODE, E.JOB_CODE)
	 = (
		SELECT DEPT_CODE, E.JOB_CODE 
		FROM EMPLOYEE 
		WHERE EMP_NAME = '노옹철'
	)
AND E.EMP_NAME <> '노옹철' ;
...
SELECT 
	EMP_NAME, DEPT_CODE, MANAGER_ID, EMP_NO, HIRE_DATE 
FROM 
	EMPLOYEE
WHERE 
	EMP_NO LIKE '87____-2______';
...
-- 2) 직급별 평균 급여
SELECT 
	JOB_CODE, TRUNC(AVG(SALARY), -4) -- 평균급여
FROM
	EMPLOYEE 
GROUP BY 
	JOB_CODE; 
 ...
 * 		-> 최종 결과(RESULT SET)에 포함될지 안될지를 지정
 * */
...
SELECT 
	E.EMP_ID, E.EMP_NAME, E.PHONE, D.DEPT_TITLE 
FROM
	EMPLOYEE E
JOIN 
	DEPARTMENT D ON (E.DEPT_CODE  = D.DEPT_ID )
...
WHERE 
		...
		-- WHERE SUB.HIRE_DATE >= TO_DATE('2010-01-01', 'YYYY-MM-DD') 
	);
...
SELECT 
	E.EMP_ID , E.EMP_NAME , E.JOB_CODE, E.EMP_NO , E.HIRE_DATE 
FROM
	EMPLOYEE E
WHERE 
	(E.DEPT_CODE, E.MANAGER_ID) = (
		SELECT 
			SUB.DEPT_CODE, SUB.MANAGER_ID
		FROM
			EMPLOYEE SUB
		WHERE 
			-- SUB.EMP_NO  LIKE '87____-2%'
			SUB.EMP_NO LIKE '87%'
...
SELECT 
	E.EMP_ID, E.EMP_NAME, NVL(D.DEPT_TITLE, '소속없음'),
...
SELECT 
	...
	FLOOR(
		MONTHS_BETWEEN(
		SYSDATE , 
		TO_DATE(19 || SUBSTR(EMP_NO, 1,6), 'RRMMDD')
		-- RRMMDD -> 50이상이면 1900년도
		-- YYMMDD -> 50 이상이면 2000년도
			) / 12
		) AS "만 나이",
	TO_CHAR(
		E.SALARY * (1 + NVL(BONUS, 0))  * 12, 'L999,999,999'
		) AS "연봉"
FROM
	EMPLOYEE E
JOIN 
