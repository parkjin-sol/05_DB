...
--> 가상 테이블인데 데이터 삽입이 성공된게 아니라 
--> 실제로는 원본 테이블에 데이터가 삽입됨

SELECT * FROM  V_COPY2 ;   -- D0 L2
SELECT * FROM  DEPT_COPY2; -- D0 NULL L2

-- V_COPY2 수정 (REPLACE 옵션 사용)
...
SELECT * FROM V_COPY2; -- DEPT_ID, DEPT_TITLE 만 보임

-- V_COPY2에 다시 INSERT 
INSERT INTO V_COPY2 
VALUES('A1', '마케팅1팀');
-- NULL을 ("KH11_PJS"."DEPT_COPY2"."LOCATION_ID") 안에 삽입할 수 없습니다

-- V_COPY2 를 이용하면  LOCATION_ID 컬럼에 INSERT 할 수 없기 때문에 
-- 자동으로 NULL이 삽입됨
--> LOCATION_ID는 NOT NULL 제약조건이 설정되어 있기 때문에 
--  NULL 삽입 불가!

-- 테이블, VIEW 구조를 파악하지 못하고 있으면 
--  VIEW를 이용한 DML 수행시 잦은 오류 발생!

/* VIEW READ ONLY OPTION 추가
 * -> VIEW ALTER 불가 -> CREATE OR REPLACE 이용
 */
CREATE OR REPLACE VIEW V_COPY2 
AS 
SELECT 
...
NOMINVALUE       -- 최소값 없음(CYCLE 과 관련된 속성)
NOCYCLE          -- 순환 X
...
/* 인덱스를 이용하지 않고 조회하기
 * - TEST_ID 컬럼 값이 'TEST50000000'인 행 조회
 * */
...
/* 인덱스를 이용해서 조회하기
 * - TEST_NO 컬럼 값이 'TEST50000000'인 행 조회
 * */
SELECT * FROM TB_IDX_TEST 
