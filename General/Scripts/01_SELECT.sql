 ...
 * 
 * - OR(또는)
 * 	 두 연산의 결과가 FALSE 일때 만 최종 결과 FALSE 
 * 	 -> 두 조건 중 하나라도 만족하는 행을
 * 		  결과 집합(RESULT SET)에 포함
 * 
 * 	 - 우선 순위 : AND > OR 
 ...
 * 
 * - OREDER BY절 보다 먼저 해석되는 
 * 	 SELECT절 별칭을
 * 	 ORDER BY절에 인식할 수 있다!  
 * 
 * ?? 그럼 SELECT 절 보다 먼저 해석되는
 ...
 * 	 컬럼의 순서도 같이 지정된다! 
 * 
 * 	 -> ORDER BY에서 컬럼 순서 이용 가능 (권장X) 
 * */

-- 급여가 400만 이상, 600만 이하인사원의
-- 사번, 이름, 급여를
-- 급여 내림차순으로 조회
SELECT 
	EMP_ID , EMP_NAME , SALARY 
