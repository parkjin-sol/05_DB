...
-- 1) ON 이용 방법 -> JOIN에 사용된 컬럼이 별도로 정렬 (2개)
SELECT
--	*
...
NATURAL JOIN JOB;
-- JOIN 컬럼명 명시 X 
--> 자동으로 같은 타입, 다른 이름의 컬럼을 찾아서 JOIN
...
SELECT 
	...
	L.LOCAL_CODE, L.LOCAL_NAME, E.SALARY 
FROM
	EMPLOYEE E 
JOIN 
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
SELECT 
	EMP_NAME, EMP_NAME, D.DEPT_TITLE 
FROM 
	EMPLOYEE E 
JOIN
	DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
...
SELECT 
	EMP_NAME , J.JOB_NAME, E.DEPT_CODE , D.DEPT_TITLE
...
SELECT 
	EMP_NAME, J.JOB_NAME, D.DEPT_TITLE, L.LOCAL_NAME 
FROM 
	EMPLOYEE E
INNER JOIN 
	DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
...
JOIN 
	LOCATION L ON (D.LOCATION_ID = L.LOCAL_CODE);
-- WHERE 
--	E.DEPT_CODE IS NOT NULL; --> INNER JOIN을 사용할 경우 굳이 안써도 됨
